...
from tree.utils import entropy, information_gain, gini_index, var_red, riro_loss
...
class DecisionTree:
    ...
    def __rido(self, examples, target_attribute, attributes, depth, weights):

        node=Node(value=None,depth=depth)
        
        if(depth == self.max_depth):
            df_end=pd.DataFrame({"Target": np.array(target_attribute),"Weights":np.array(weights).flatten()})
            df_end=df_end.groupby("Target").sum()
            max_index=df_end["Weights"].idxmax()
            node.value = "Class "+str(df_end.loc[max_index].name)
            return node
        ...
        for column in examples.columns:
            ...
            example_copy['weights']=weights_copy
            ...
            for i in range(len(sorted_atrribute)-1):
                ...
                if( self.criterion == 'information_gain'):
                    ...
                    inf_gain_split=information_gain(sorted_atrribute, col_, example_copy['weights'])
                    ...
                    for val in col_.unique():
                        ...
                        weights_sub=example_copy['weights'].copy()
                        weights_sub=weights_sub[col_==val]
                        inf_gain_split += gini_index(targ_sub, weights_sub)*(len(targ_sub)/len(col_))
                    if(inf_gain_split>max_gain_split):
                        max_gain_split=inf_gain_split
                        split_best=split
        ...
        weights_val_0 = weights.loc[example_val_0.index].copy()
        weights_val_1 = weights.loc[example_val_1.index].copy()

 
        child_node_0 = self.__rido(example_val_0, target_attribute_val_0, attributes,depth+1, weights_val_0)
        child_node_1 = self.__rido(example_val_1, target_attribute_val_1, attributes,depth+1, weights_val_1)
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series, weights: pd.Series = pd.Series([])  ) -> None:
        """
        ...
        #Real Input Discrete output
            ...
            self._root_node = self.__rido(examples = X, target_attribute = y, attributes = X.columns.values, depth=0, weights = weights)
            return self._root_node
        ## Real input real output
        elif( str(X[col].dtype) != "category" and str(y.dtype) != "category" ):
